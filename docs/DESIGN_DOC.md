...
> **Implementation status:** this document was originally written as a build
> spec, in a Python/FastAPI framing, before any of it existed. The actual
> implementation is Node.js / Express / LangGraph.js (see `README.md` and
> `src/server/`), and it now implements the architecture described below —
> hybrid retrieval, reranking, the grader/rewrite loop, the hallucination
> check, the Postgres checkpointer, the human-approval gate — for real, with
> two deliberate deltas: **reranking is LLM-based** rather than a
> self-hosted BGE cross-encoder (§6), and **evaluation is a native Node
> harness** rather than Python RAGAS (§10), both because pulling in a
> second language runtime for one piece of a Node project wasn't a trade
> worth making. Read this document for the *why*; read the README and the
